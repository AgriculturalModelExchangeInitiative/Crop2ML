...
namespace SiriusQualityPhenology.Strategies
{
	...
	#region pre/post conditions management		
			...
			public string TestPreConditions(SiriusQualityPhenology.PhenologyState phenologystate,SiriusQualityPhenology.PhenologyState phenologystate1, string callID)
			{
				try
				{
					...
					if(r2.ApplicableVarInfoValueTypes.Contains( SiriusQualityPhenology.PhenologyStateVarInfo.Fixphyll.ValueType)){prc.AddCondition(r2);}
	...
	#region Model
			...
			{				
				...
				//Code written below will not be overwritten by a future code generation
                if (phenologystate.LeafNumber < Ldecr) phenologystate.Phyllochron = phenologystate.Fixphyll * Pdecr;
                else if (phenologystate.LeafNumber >= Ldecr && phenologystate.LeafNumber < Lincr) phenologystate.Phyllochron = phenologystate.Fixphyll;
                else phenologystate.Phyllochron = phenologystate.Fixphyll * Pincr;
