...
public class STMPsimCalculator extends FWSimComponent
{
	...
	public HashMap<String, FWSimVariable<?>> createVariables()
	{
		...
		addVariable(FWSimVariable.createSimVariable("SoilTempArray", "Array of temperature ", DATA_TYPE.DOUBLEARRAY,
				CONTENT_TYPE.out, "http://www.wurvoc.org/vocabularies/om-1.8/degree_Celsius", -20, 40, null, this));
	...
	protected void init()
	{
    Double[] Z = (Double[]) cSoilLayerDepth.getValue();
	...
	protected void process()
	{
		...
		for (int i = 0; i < SoilTempArray.getValue().length; i++)
		{
			...
			double RATE = ZD / (ZD + exp(-.8669 - 2.0775 * ZD)) * (cAVT.getValue() - iSoilSurfaceTemperature.getValue());
				//RATE = Rate of change of STMP(ISL) (°C)
			SoilTempArray.getValue()[i] = XLAG * SoilTempArray.getValue()[i] + XLG1 * (RATE + iSoilSurfaceTemperature.getValue());
			Z1 = tZp[i];
		}
